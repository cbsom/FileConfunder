...
namespace Confunder
{
    internal static class Program
    {
        ...
        private static int _bufferLength = 4096;
...
USAGE: 
    ...
    ""-action"" which action to run. The action can be any one of the following values:
        ...
        * ""setkey"" - update the key stored securely on this machine for future runs. The -key argument must be supplied with this action.
    ...
    [-key] Any text key to use for confunding and unconfunding. The app derives a base64 key from this text.
    [-silent] No console output and on Windows the command window is hidden.";
        ...
        private static bool SetKey()
        {
            if (_key == null)
            {
                Console.Error.WriteLine("No key was supplied. Use -key <text> with -action setkey.");
                return false;
            }
        ...
        private static bool ConfundFile(string path, bool unconfund)
        {
            bool success = false;
            try
            {
                ...
                buf = EncryptData(buf, ungarble: unconfund);
                file.Position = 0;
        ...
        private static bool RemoveConfundMarkFromFile(string path)
        {
            try
            {
        ...
        private static bool IsFileConfunded(string path)
        {
        ...
        {
            return EncryptData(data, _key, ungarble);
        }
        ...
        public static void RemoveBytesFromStart(FileStream fs, int bytesToRemove)
        {
            ...
            if (bytesToRemove >= fileLength)
            {
                // If removing more than the file size, just clear the file
                fs.SetLength(0);
                return;
            }

            // Buffer size for shifting data (4096 to 65536 bytes is ideal)
            ...
            while ((bytesRead = fs.Read(buffer, 0, buffer.Length)) > 0)
            {
                ...
                // Move pointer to write position and write
                fs.Position = writePosition;
                fs.Write(buffer, 0, bytesRead);

                // Advance write position and restore read position
                writePosition = fs.Position;
                fs.Position = readPosition;
            }

            // Truncate the leftover duplicate data at the end
            fs.SetLength(writePosition);
        }
